...
void virtual_kdb(unsigned short int *pixels,int vx,int vy)
{
   ...
   for(x=0;x<NPLGN;x++)  // 10 letters horiz.
   {
      for(y=0;y<NLIGN;y++)  // 5 letters vert.
      {
         ...
         Draw_text(pix,XBASE0-2+x*XSIDE ,YBASE0+YSIDE*y,coul, BKGCOLOR ,fscale, fscale,20,
   ...
   Draw_text(pix,XBASE0-2+vx*XSIDE ,YBASE0+YSIDE*vy,RGB565(30,57,0), BKGCOLOR ,fscale, fscale,20,
